...
import { ArrowLeft, Printer, CreditCard, FileText, Download } from 'lucide-react';
...
export default function KIBPrint({ patient, onBack, kibSettings, initialFormat = 'card', hideFormatToggle = false }: KIBPrintProps) {
  ...
  const handleDownloadImage = async () => {
    ...
    try {
      ...
      link.href = image;
      link.download = `KIB_${patient.noRm}.png`;
      link.click();
    } catch (err) {
    ...
    <div className="flex-1 p-8 md:p-12 flex flex-col items-center print:bg-white print:p-0">
      <div className="w-full max-w-3xl mb-8 flex justify-between items-center print:hidden">
        ...
        <div className="flex space-x-2">
          {printFormat === 'card' && (
            <button onClick={handleDownloadImage} className="flex items-center space-x-2 bg-emerald-600 text-white px-4 py-2.5 rounded-md text-[13px] font-semibold hover:bg-emerald-700 transition shadow-sm">
      ...
      {printFormat === 'card' ? (
        /* Card Container */
        <div className="bg-white p-10 rounded-xl border border-[#E5E7EB] shadow-sm max-w-2xl w-full print:shadow-none print:border-none print:p-0 print:bg-transparent flex flex-col items-center">
