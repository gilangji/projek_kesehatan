...
export default function Settings({ onBack, kibSettings, onUpdateKibSettings }: SettingsProps) {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>, type: 'logo' | 'secondaryLogo' | 'background' | 'frontBackground') => {
    ...
    try {
      ...
      const newSettings = { 
        ...localSettings, 
        [type === 'logo' ? 'logoUrl' : type === 'secondaryLogo' ? 'secondaryLogoUrl' : type === 'frontBackground' ? 'frontBackgroundUrl' : 'backgroundUrl']: data.publicUrl 
      };
      ...
      const { error: dbError } = await supabase.from('settings').upsert({
        ...
        front_background_url: newSettings.frontBackgroundUrl
      });
  ...
  return (
    <div className="flex-1 flex flex-col">
      ...
      <main className="flex-1 p-8 lg:p-12 max-w-4xl mx-auto w-full">
        <header className={`mb-8 flex flex-col items-start ${kibSettings.backgroundUrl ? 'space-y-3' : ''}`}>
          ...
          <p className={kibSettings.backgroundUrl ? "text-[14px] font-medium text-[#4B5563] bg-white/95 px-5 py-2 rounded-full shadow-sm backdrop-blur-sm" : "text-[15px] text-[#6B7280]"}>
            Atur logo dan latar belakang Kartu Identitas Berobat secara permanen.
          </p>
        </header>
              ...
              {localSettings.frontBackgroundUrl ? (
                <div className="flex flex-col items-center justify-center p-6 bg-white border border-[#E5E7EB] rounded-md mb-4">
                  <div 
                    ...
                    style={{ backgroundImage: `url(${localSettings.frontBackgroundUrl})` }}
                  ></div>
                  <span className="text-xs font-semibold text-emerald-600 flex items-center gap-1 bg-emerald-50 px-2 py-1 rounded">
                    <Lock className="w-3 h-3" /> Permanen
                  </span>
                ...
                <div className="mb-4">
                  ...
                  <input 
                    type="file" 
                    accept="image/*"
                    onChange={(e) => handleImageUpload(e, 'frontBackground')}
                    disabled={isUploading}
                    className="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-[#E0E7FF] file:text-[#2563EB] hover:file:bg-blue-100 cursor-pointer disabled:opacity-50"
                  />
                </div>
